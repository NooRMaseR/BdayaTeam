    ...
    const saveSettings = () => {
        ...
        const submissionPromise = API.PUT("/api/organizer/settings/", {
            ...
            bodySerializer(body) {
                ...
                if (body?.organizer_can_edit) body.organizer_can_edit.forEach(f => fd.append("organizer_can_edit", f));
                if (body?.site_image) fd.append("site_image", body.site_image);
                if (body?.hero_image) fd.append("hero_image", body.hero_image);
