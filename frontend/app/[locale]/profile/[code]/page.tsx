...
export default async function ProfilePage({ params }: { params: Promise<{ code: `${string}-${number}` }> }) {
    ...
    return (
        <BodyM>
            <div className="min-h-screen w-full bg-slate-50/50 py-8 px-4">
                <div className="max-w-5xl mx-auto flex flex-col gap-8">
                    
                    <Paper elevation={0} className="w-full overflow-hidden rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        <div className="px-6 sm:px-10 pb-8 relative">
                            ...
                            <div className="grid grid-cols-3 divide-x divide-slate-100 bg-slate-50/50 rounded-xl border border-slate-100">
                                <Stat label={tr("abs")} value={user.absents} colorClass={user.absents > 3 ? "text-red-500" : "text-slate-900"} />
