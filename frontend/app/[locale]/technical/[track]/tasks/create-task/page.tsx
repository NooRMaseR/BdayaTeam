    ...
    return (
        <div className='flex justify-center items-center h-[80svh] w-full'>
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col h-max lg:w-[50%] md:w-[50%] sm:w-[90%] w-[90%] gap-4'>
                <h2 className='lg:text-4xl sm:text-3xl'>{tr('taskTitle')}</h2>
                <TextField label={tr('taskNum')} {...register("task_number", { required: true })} required fullWidth error={!!errors.task_number} helperText={errors.task_number?.message} type='number' inputMode='numeric' />
                ...
                <Button type='submit' variant='contained' loadingPosition='start' loading={isLoading}>{tr('create')}</Button>
            </form>
        </div>
    )
}
